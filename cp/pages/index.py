...
class State(rx.State):
    ...
    def create_new_cluster(self, form_data: dict):
        ...
        # create new entry in clusters table with status "REQUESTED"
        # new_cluster = db.create_cluster(
        #     form_data["cluster_id"], "REQUESTED", "root", "root"
        # )

        # # start job to create a cluster
        # # the job will set the  status to "DEPLOYING"
        # # once complete will set the status to "OK"
        # job_id = runner.create_cluster(form_data)
...
def get_cluster_row(cluster: Cluster):
    ...
    return rx.table.row(
        rx.table.cell(
            rx.link(
                cluster.cluster_id,
                on_click=State.get_cluster(cluster.cluster_id),
                href=f"/cluster/{cluster.cluster_id}",
            )
        ),
...
@rx.page(route="/", title="Home", on_load=State.fetch_all_clusters)
@template
def index():
    return rx.flex(
        rx.hstack(
