...
class State(AuthState):
    ...
    def get_backup(self, backup_path: str):
        ...
        try:
            self.backup_details = ClusterBackupsService.load_backup_details(
                self.cluster_id,
    ...
    def initiate_restore(self, form_data: dict):
        try:
            job_id = ClusterBackupsService.request_cluster_restore(
            ...
            async with self:
                try:
                    snapshot: ClusterBackupsSnapshot | None = (
                        ClusterBackupsService.load_cluster_backups_snapshot(
                            self.cluster_id,
                            list(self.webuser.groups),
                            self.is_admin,
                        )
