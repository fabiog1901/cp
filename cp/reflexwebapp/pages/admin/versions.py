...
class State(AuthState):
    ...
    def remove_version(self, v: Version):
        try:
            VersionsService.delete_version(v.version, self.webuser.username)
    ...
    def submit_new_version(self):
        ...
        try:
            VersionsService.create_version(self.version, self.webuser.username)
        except Exception as e:
            return NotifyState.show("Error", str(e))
    ...
    async def start_bg_event(self):
        ...
        while True:
            ...
            async with self:
                try:
                    self.versions = VersionsService.list_versions()
                except Exception as e:
                    self.is_running = False
                    return NotifyState.show("Error", str(e))
