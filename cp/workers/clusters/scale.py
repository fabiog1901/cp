...
def scale_cluster(
    ...
    current_cluster = cluster.get_cluster(cluster_scale_request.name, [], True)
...
def scale_cluster_worker_entry(
    ...
    try:
        ...
        cluster.update_cluster(csr.name, requested_by, status=ClusterState.SCALE_FAILED)
...
def scale_cluster_worker(
    ...
    if csr.node_count < current_cluster.node_count:
        ...
        if job_status != "successful":
            cluster.update_cluster(csr.name, requested_by, status="SCALE_FAILED")
            return
