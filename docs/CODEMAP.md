...
| Workers | `cp/workers/commands.py`, `cp/workers/remote/*.py`, `cpkit/cpkit/jobs/` |
...
| Concern | Entry points |
| --- | --- |
| App auth dependencies | `cpkit` exports consumed by CP routers |
...
| Add a framework metadata table | `cpkit/resources/ddl.sql`, then the relevant `cpkit` package |
...
| Change schema defaults/dev data | `cpkit/resources/ddl.sql`, `resources/ddl.sql`, `resources/init.sql`, `resources/.dev-setup.sql` |
...
- `rg "CREATE TABLE cpkit" cpkit/resources/ddl.sql`
